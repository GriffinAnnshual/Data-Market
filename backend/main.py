...
def send():
    try:
        ...
        <body>
            ...
            <a href="http://127.0.0.1:5000/verify/email?verify={1}&email={2}">
...
def verify():
    try:
        ...
        if data.get("email") == email:
            response = make_response(redirect("http://localhost:5173/register"))
            response.set_cookie('email_verified', 'true')
            return response
        else:
